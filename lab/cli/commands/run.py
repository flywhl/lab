...
async def run(
    ...
    log_file: Annotated[
        Optional[Path], 
        typer.Option("--log-file", help="Write logs to file")
    ...
    try:
        ...
        with ui.create_progress() as progress:
            task = progress.add_task("Running experiments...", total=len(plan.ordered_experiments))
            runtime = Runtime(run_service=RunService(
                project_run_repo=InMemoryProjectRunRepository(),
                experiment_run_repo=InMemoryExperimentRunRepository()
            ))
            results = await runtime.start(plan, progress=task)
